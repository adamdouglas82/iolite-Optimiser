...
* **Draft Multi-Line Release Notes**:
  > [!IMPORTANT]
  > The GitHub Actions workflow (`package-release.yml`) uses `git log -1 --pretty=format:"%B"` to extract the GitHub release notes body. Always commit with a **full multi-line commit message** starting with the title format `Release v<version>: <summary>` (followed by section headers and bullet points) so the GitHub release displays complete release notes.
